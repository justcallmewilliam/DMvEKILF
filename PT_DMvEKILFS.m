function [w1, P] = PT_DMvEKILFS(classOne , classTwo , C1, C2, inputInf)
    ...
    for view = 1 : inputInf.view
       ...
       P{view} = rand(org_dim(view), inputInf.dim); 
...
function z_i = update_zi( P, w, w0, X, y, h, iter, num_sample, inputInf, C2)
    ...
    [z_i, ~, ~, ~,~] = quadprog(H, c, A, b, [], [], []);
    
end

function P_v = update_P(P_v, Z, X_v, inputInf, C1)
    A = C1 * construct_D(P_v); 
     
    B = zeros(size(X_v, 1), size(X_v, 1));
    C = zeros(size(X_v, 1), inputInf.dim);
    for i = 1 : size(X_v, 2)
        B = B + (1/(size(X_v, 2) * inputInf.view)) * X_v( : , i) * X_v( : , i)';
        C = C + (1/(size(X_v, 2) * inputInf.view)) * X_v( : , i) * Z( : , i)' ;
    end
    P_v = inv(B + A) * C;
end
